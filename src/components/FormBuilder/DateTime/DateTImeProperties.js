...
const DateTImeProperties = ({
  ...
  const [oldInitialDate, setOldInitialDate] = useState(null);

  const handleCheckboxChange = (event) => {
    placeholderChange(
      { target: { value: !element?.useCurrentDate } },
      "useCurrentDate"
    );
  };
  ...
  useEffect(() => {
    if (element?.useCurrentDate) {
      let newDate = new Date();
      placeholderChange({ target: { value: newDate.toISOString() } }, "date");
    }
  }, [element?.useCurrentDate]);
            ...
            {!element?.selectsRange && (
              <div className="form-input">
                ...
                <DatePicker
                  ...
                  disabled={element?.useCurrentDate}
                  // timeClassName={(time) => "text-danger"}
                />
                <div
                  className="form-checkbox-currentdate"
                  style={{ marginTop: "12px" }}
                >
                  <label>
                    <input
                      type="checkbox"
                      id="currentDate-checkbox"
                      checked={element?.useCurrentDate}
