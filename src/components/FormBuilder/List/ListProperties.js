...
const ListProperties = ({
  ...
  const renderDataModelOptions = () => {
    return dataModels?.map((item) => {
      return (
        <option
          selected={
            item.resourceName === element.dataGridProperties.parentDataModelName
              ? true
              : false
          }
  ...
  const renderNestedDataModelOptions = () => {
    let dataModelOptions = element.dataGridProperties.parentDataModelCols?.map(
      (entity) => {
        return entity.name;
      }
    );
    // console.log("dmopt", dataModelOptions);
    return dataModels?.map((item) => {
      if (
        dataModelOptions?.includes(
          item.resourceName.toLowerCase().replace(".java", "")
        )
      ) {
        return (
          <option
        ...
        if (item?.stack?.length) {
          ...
          if (stackTemp?.length) {
            ...
            addElementIntoList(
              ...
              item.dataGridProperties.parentDataModelName
            );
            return {
              ...item,
              dataGridProperties: {
  ...
  const addElementIntoList = (
    ...
    processVariableName =
      parentDataModelName.replace(".java", "") +
      "." +
      dataModelFullName.replace(".java", "") +
      ".0." +
      dataModelEntityProperties[0].name;
  ...
  const handleParentDataModelChange = (event) => {
    const temp = layout.layout.map((item) => {
      if (item.edit) {
        if (item?.stack?.length) {
          const stackTemp = item.stack.filter((stack) => stack.edit === true);
              ...
              stack: item.stack?.map((stack) => {
                if (stack.edit) {
                  return {
                    ...
                    dataGridProperties: {
                      ...item.dataGridProperties,
                      parentDataModelName: event.target.value
                        ? event.target.value
                        : null,
                      parentDataModelCols: [],
                    },
    ...
    setLayout(layout);
    // fetchMetaContent(event.target.value);
    fetchChildMetaContent(event.target.value);
  };
  ...
  const fetchChildMetaContent = (val) => {
    if (val) {
      const postData = {
        workspaceName: localStorage.getItem("workspace"),
        miniAppName: localStorage.getItem("appName"),
                      ...
                      stack: item.stack?.map((stack) => {
                        if (stack.edit) {
                          return {
                            ...
                            dataGridProperties: {
                              ...stack.dataGridProperties,
                              parentDataModelCols: jsonData?.map((data) => {
                                return {
                                  ...data,
                                  required: false,
                                };
    ...
    <>
      <form>
        ...
        <div className="form-input mt-3">
          <label>Please select a parent Data-Model</label>
          <select
            id="dataGridProperties-parentdataModel-select"
            onChange={handleParentDataModelChange}
          >
        ...
        {/* {element.dataGridProperties.cols && (
          <div className="form-input mt-3">
            <label>Please select the columns</label>
            <div className="columnfields px-2">
              <div className="columnname">
                <label>Column</label>
                <SortableList
                  data={element.dataGridProperties.cols.filter(col => col.isCollection == true)}
                  onSortEnd={onSortEnd}
